...
export default function MultiResultItem({shows, mediaType}) {
      ...
      {shows && shows.map(show => (
        <li key={show.id} onClick={() => {
          ...
          <div className='media'>
            ...
            {show.media_type && show.media_type === 'movie' ? <p className='media-name flex'><span><img src={iconMovie} alt="icon movie" /></span>{show.media_type}</p> : <p className='media-name flex'><span><img src={iconTV} alt="icon tv" /></span>{show.media_type} Series</p>}
          </div>
